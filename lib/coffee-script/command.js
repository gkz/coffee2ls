...
var $0, $0, additionalArgs, additionalArgs, additionalArgs, arg, args, CoffeeScript, concat, escodegen, foldl, formatOptions, fs, humanReadable, input, input, inspect, leftColumnWidth, longOptionArguments, longParameterArguments, lscodegen, match, match, match, maxWidth, numberLines, o, o, o, Optimiser, optionArguments, optionMap, optionRows, options, opts, opts, parameterArguments, parameterRows, path, pkg, positionalArgs, positionalArgs, Preprocessor, processInput, reLongOption, reLongParameter, reShortOptions, reShortOptionsShortParameter, reShortParameter, row, rows, shortOptionArguments, shortParameterArguments, uglifyjs, wrap;
fs = require('fs');
path = require('path');
...
if (!(options.compile || options.js || options['source-map'] || options.parse || options['eval'] || options.lscodegen))
  if (!(null != escodegen)) {
...
if (1 !== options.parse + options.compile + (null != options.js ? options.js : 0) + (null != options['source-map'] ? options['source-map'] : 0) + (null != options['eval'] ? options['eval'] : 0) + (null != options.lscodegen ? options.lscodegen : 0) + (null != options.repl ? options.repl : 0)) {
...
if (options.lscodegen && !(null != lscodegen)) {
...
if (options.help) {
  ...
  processInput = function (err) {
    var d, domain, e, evalFn, handleNodes, js, jsAST, result, sourceMap;
    if (null != err)
      throw err;
    result = null;
    ...
    if (options.lscodegen) {
      handleNodes = function (node, o) {
        var child, k, name;
        if (node.className === 'AssignOp' && node.assignee.className === 'Identifier') {
        ...
        for (k in node) {
          child = node[k];
          if (!(null != child['instanceof']))
            continue;
          handleNodes(child, o);
        }
        return function (accum$) {
          for (var i$6 = 0, length$6 = node.listMembers.length; i$6 < length$6; ++i$6) {
            k = node.listMembers[i$6];
            accum$.push(function (accum$1) {
              for (var i$7 = 0, length$7 = node[k].length; i$7 < length$7; ++i$7) {
                child = node[k][i$7];
                accum$1.push(handleNodes(child, o));
              }
              return accum$1;
            }.call(this, []));
          }
      ...
      handleNodes(result, []);
      try {
        result = lscodegen.generate(result);
      } catch (e$2) {
        e = e$2;
